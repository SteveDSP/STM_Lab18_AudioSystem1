...
//So this will transfer 512 * int16 per block 1024 bytes
#define RECORD_BUFFER_SIZE  2048

//These are defined in main.c
//They form the system really, hence global.

/* Buffers used to stream the recorded PCM samples towards the audio codec. */
extern int16_t RecordBuffer[RECORD_BUFFER_SIZE];
extern int16_t PlaybackBuffer[RECORD_BUFFER_SIZE];
//extern int16_t CircularBuffer[RECORD_BUFFER_SIZE*3];
extern volatile uint32_t  audio_rec_buffer_state; //what is the current state of the buffer? see enum above

extern TS_StateTypeDef TS_State; //Touch Screen state
extern uint32_t ts_status; //Touch Status
extern float coeffOne; //Default float test volume value
extern float coeffTwo; //Default float test filter coeff
extern float coeffOne_inc; //fractional inc
extern float coeffTwo_inc; //fractional inc

extern int16_t CurrentLsample, CurrentRsample;
...
/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */
// extern const float INT16_TO_FLOAT; //declared in AudioProcess.cpp ..might not need this extern
/* USER CODE END EM */
...
uint8_t CheckForUserInput(void); //Will return a 1 or 0
